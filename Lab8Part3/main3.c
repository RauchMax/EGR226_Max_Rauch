...
void changeSpeed(void);                 // prototype to change the speed
void timer_init(void);                  // prototype for timer A initialization
void SysTick_delay (uint16_t delay);    // prototyping the systick function
uint8_t num, pressed;                   //making global variables for reading the keypad
...
void main(void)
{
    WDT_A->CTL = WDT_A_CTL_PW | WDT_A_CTL_HOLD;     // stop watchdog timer
     ...
     timer_init();     //initialize the timer A


    while ( 1 )
        {
        pressed =  Keypad_Read();   // Call Function to read Keypad
        if ( pressed )              //checking the button state
        changeSpeed();              //if a key is pressed change the speed
...
void changeSpeed(void){
    int DC;
        ...
        if(num == 1){   //if 1 is pressed set duty cycle to .1
           DC = 9000;
        }

        if(num == 2){   //if 2 is pressed set duty cycle to .2
           DC = 8000;
        }

        if(num == 3){   //if 3 is pressed set duty cycle to .3
           DC = 7000;
        }

        if(num == 4){   //if 4 is pressed set duty cycle to .4
           DC = 6000;
        }

        if(num == 5){   //if 5 is pressed set duty cycle to .5
            DC = 5000;
        }

        if(num == 6){   //if 6 is pressed set duty cycle to .6
            DC = 4000;
        }

        if(num == 7){   //if 7 is pressed set duty cycle to .7
            DC = 3000;
        }

        if(num == 8){   //if 8 is pressed set duty cycle to .8
            DC = 2000;
        }

        if(num == 9){   //if 9 is pressed set duty cycle to .9
            DC = 1000;
        }
        if(num == 11){   //if 11 is pressed set duty cycle to 0
            DC = 10000;
        }

        if(num == 12){   //if 12 is pressed set duty cycle to 1
            DC = 0;
        }

        if(num == 10){   //if 10 is pressed set duty cycle to 1
            DC = 0;
        }
